...
if (!empty($_POST)) {
    ...
    if (!$erreur) {
        ...
        $sql = "SELECT COUNT(*) as nb FROM PLO_PALANQUEE WHERE `PLO_DATE` = '".$date."' AND `PloMatinApresmidi` = '".$periode."'";
        $db->LireDonneesPDO1($sql, $Palanquee);
        $nbPalanquee = ($Palanquee[0]['nb'])+1;
        echo $nbPalanquee;

        //Ajoute une nouvelle Plongee dans la Base de Donnée
        $sql = "INSERT INTO PLO_PALANQUEE (PLO_DATE, PloMatinApresmidi, PAL_NUM, PAL_PROFONDEUR_MAX, PAL_DUREE_MAX, PAL_HEURE_IMMERSION, PAL_HEURE_SORTIE_EAU, PAL_PROFONDEUR_REELLE, PAL_DUREE_FOND)"
